...
export default function FileTree({ nodes, depth = 0 }: Props) {
  return <div className="space-y-1">{nodes.map((node) => <TreeNode key={node.id} node={node} depth={depth} />)}</div>;
}
...
function TreeNode({ node, depth }: { node: WorkspaceNode; depth: number }) {
  ...
  const { openFile, loadDirectory } = useWorkspaceStore();

  const toggleFolder = async () => {
    const nextOpen = !open;
    setOpen(nextOpen);
    if (nextOpen && !node.loaded) await loadDirectory(node.path, node.id);
  };

  if (node.type === "folder") {
    return <div><button onClick={() => void toggleFolder()} className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-purple-50 text-sm">{open ? <ChevronDown size={15} /> : <ChevronRight size={15} />}<Folder size={15} className="text-violet-500" />{node.name}</button>{open && node.children && <div className="ml-5"><FileTree nodes={node.children as WorkspaceNode[]} depth={depth + 1} /></div>}</div>;
  }
