...
import { Bell, CheckCheck, ChevronRight, CircleHelp, Cloud, Command, FolderOpen, GitBranch, GitFork, Minimize2, Moon, Search, Settings, Sparkles, Sun, UserRound, X } from "lucide-react";
...
export default function TopBar({ onToggleTerminal, terminalCollapsed }: TopBarProps) {
  ...
  return <header className="topbar"><div className="topbar-left"><div className="brand-lockup"><img src={zenithMark} alt="Zenith" /><strong>Zenith</strong></div><div className="app-menu-row" aria-label="Application menu">{Object.keys(menus).map((name) => <AppMenu key={name} label={name} items={menus[name]} open={activeMenu === name} onToggle={() => setActiveMenu((current) => current === name ? null : name)} onClose={() => setActiveMenu(null)} />)}</div><span className="topbar-divider" /><div className="topbar-menu-anchor" ref={projectMenuRef}><button className="project-switcher" onClick={() => setProjectMenuOpen((value) => !value)} aria-expanded={projectMenuOpen}><FolderOpen size={16} /><span>{rootName}</span><span className="chevron">⌄</span></button>{projectMenuOpen && <div className="zenith-popover project-menu" role="menu"><button role="menuitem" onClick={() => { void openFolder(); setProjectMenuOpen(false); }}><FolderOpen size={15} />Open Folder</button><button role="menuitem" onClick={() => { void closeFolder(); setProjectMenuOpen(false); }} disabled={!rootPath}><X size={15} />Close Folder</button></div>}</div><button className="branch-switcher" title="Open Source Control" onClick={() => togglePanel("git")}><GitBranch size={15} /><span>Source Control</span></button></div>
    ...
    <div className="topbar-actions"><Cloud className="topbar-cloud" size={19} aria-label="Workspace available offline" /><button className="ai-toggle" onClick={toggleAIPanel} title="Toggle Zenith AI"><Sparkles size={15} /><span>AI</span></button><button className="top-icon" title="Toggle light and dark preference" aria-label="Toggle light and dark preference" onClick={toggleTheme}>{theme.appearanceMode === "dark" || (theme.appearanceMode === "mixed" && theme.monaco.mode === "dark") ? <Sun size={17} /> : <Moon size={17} />}</button><div ref={notificationRef}><button className="top-icon notification-button" title="Notifications" aria-label="Notifications" aria-expanded={notificationOpen} onClick={() => { setNotificationOpen((value) => !value); markNotificationsRead(); }}><Bell size={17} />{unread > 0 && <i>{unread > 9 ? "9+" : unread}</i>}</button>{notificationOpen && <div className="zenith-popover notification-menu"><header><strong>Notifications</strong><button onClick={clearNotifications} disabled={!notifications.length}><CheckCheck size={14} />Clear</button></header>{notifications.length ? notifications.slice(0, 6).map((item) => <p key={item.id}>{item.message}</p>) : <p className="empty-notifications">You’re all caught up.</p>}</div>}</div><button className="top-icon" title="Help" aria-label="Help" onClick={() => openDialog("help")}><CircleHelp size={17} /></button><button className="top-icon" title="Settings" aria-label="Settings" onClick={() => openSettings()}><Settings size={17} /></button><div ref={accountRef}><button className="account-button" title="Account" aria-label="Account" onClick={() => setAccountOpen((value) => !value)}><UserRound size={16} /></button>{accountOpen && <div className="zenith-popover account-menu"><header><span className="account-avatar"><UserRound size={16} /></span><div><strong>Not signed in</strong><small>Local desktop workspace</small></div></header><button onClick={() => { openDialog("auth", "signin"); setAccountOpen(false); }}>Sign in</button><button onClick={() => { openDialog("auth", "signup"); setAccountOpen(false); }}>Create account</button><button onClick={() => { openDialog("github"); setAccountOpen(false); }}><GitFork size={14} />Connect GitHub</button></div>}</div>{window.zenithDesktop && <div className="window-controls" aria-label="Window controls"><button title="Minimize window" onClick={() => void window.zenithDesktop?.minimizeWindow()}><Minimize2 size={16} /></button><button title="Maximize or restore window" onClick={() => void window.zenithDesktop?.toggleMaximizeWindow()}>□</button><button title="Close window" onClick={() => void window.zenithDesktop?.closeWindow()}><X size={16} /></button></div>}</div>
