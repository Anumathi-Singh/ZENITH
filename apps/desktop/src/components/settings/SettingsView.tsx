...
export default function SettingsView() {
  ...
  const { session, providers, loading: authLoading, signOut } = useAuthStore();
  ...
  switch (settingsCategory) {
    ...
    case "GitHub & Accounts": content = <><p className="settings-lead">GitHub is optional. Local files, terminal, editor, and Git continue to work while signed out.</p>{session ? <><div className="settings-account-card">{session.user.avatarUrl && <img src={session.user.avatarUrl} alt="" />}<span><strong>{session.user.name || session.user.login}</strong><small>GitHub · @{session.user.login}</small></span></div><button className="settings-action" disabled={authLoading} onClick={() => void signOut("github")}><Cloud size={17} />Disconnect GitHub <small>Connected</small></button></> : <button className="settings-action" disabled={!providers.github.configured || !providers.github.secureStorageAvailable} onClick={() => openDialog("github")}><Cloud size={17} />Connect GitHub <small>{!providers.github.configured ? "Not configured" : providers.github.secureStorageAvailable ? "Disconnected" : "Secure storage unavailable"}</small></button>}<button className="settings-action" disabled><CircleUserRound size={17} />Zenith Account <small>Unavailable</small></button><button className="settings-action" disabled><CircleUserRound size={17} />Google <small>Unavailable</small></button><button className="settings-action" disabled><CircleUserRound size={17} />Microsoft <small>Unavailable</small></button></>; break;
